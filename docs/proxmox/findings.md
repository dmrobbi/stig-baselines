...
| AF-0003 | PVE-STIG-0050 | Password quality policy (pwquality minlen >= 12) | CAT II | CLOSED | Resolved 2026-09-23: `minlen = 12` set + libpam-pwquality verified on all 4 nodes; owner supplied the compliant root@pam password — set on pve2/3/4 (PVE UI logins verified via the API); pilot config-only (its documented password stays) | root SSH key-only; pve2/3/4 UIs now password-gated | Dawn | owner decision delivered via .new_password |
